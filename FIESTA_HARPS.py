...
global V, idx_ccf, v
STAR 	= 'HD23249'
RVC  	= np.loadtxt('../' + STAR + '/info.txt')[0]
RVW 	= np.loadtxt('../' + STAR + '/info.txt')[1]
V       = np.arange(RVC-RVW, RVC+RVW+0.25, 0.25)
...
def power_phase_freq(file):
	...
	v           = v0 + np.arange(len(CCF)) * delta_v	
	idx_ccf 	= (v <= max(V)) & (v >= min(V))
	ccf 		= CCF[idx_ccf]
	popt, pcov 	= curve_fit(gaussian, V[:-1], ccf)
	rv_gauss 	= popt[1] * 1000
...
'''
FILE 							= sorted(glob.glob('../' + STAR + '/3-ccf_fits/*.fits'))
N_file 							= len(FILE)
...
def freq_of_each_section(power_tpl, freq, number_of_section):
	...
	Divide this range into number_of_sectiontions to study RV shifts in each frequency section. 
	...
	'''
	upper_freq 	= 0.145						# higher limit of frequency range
	idx 		= (freq <= upper_freq)
	n_idx 		= len(freq[idx])
	...
	# Section 1: [0, freq_i[1]; number_of_sectiontion 2: [freq_i[1], freq_i[2]]...
...
for number_of_section in Section:				# equally divide power spectrum into number_of_section

	print('\n%d' %number_of_section)

	RV_gauss 	= np.zeros(N_file)							# RV derived from a Gaussian fit
	RV_FT 		= np.zeros((N_file, number_of_section))
	delta_RV 	= np.zeros((N_file, number_of_section))
	...
	train_x, test_x, train_y, test_y = train_test_split(delta_RV, RV_gauss, test_size=0.33, random_state=42)
